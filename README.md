...
— a separate import workstream is in flight in `src/import/docx/`).
...
1. **Pretext is confined to `src/layout/`.** One adapter surface; API churn in
...
|---|---|---|
| `src/model/` | Document tree, ops + inverses, text/grapheme utils, named styles | `document.ts` `ops.ts` `text.ts` `stylesheet.ts` |
| `src/layout/` | pretext integration, line caches, pagination + break rules, floats, tables, bands, geometry queries | `engine.ts` `geometry.ts` `prepareCache.ts` `layoutTree.ts` `metrics.ts` |
| `src/paint/` | Virtualized per-page canvases, DPR, selection/search rects, caret overlay, band dimming | `renderer.ts` |
| `src/input/` | IME proxy, selection controller, object frames/resize, clipboard (HTML+plain), keymap | `imeProxy.ts` `selectionController.ts` `objectController.ts` `clipboard.ts` `keymap.ts` |
| `src/editor/` | Commands (pure), transactions, undo manager | `commands.ts` `state.ts` `undo.ts` |
| `src/a11y/` | Screen-reader mirror + live region | `mirror.ts` |
| `src/import/docx/` | .docx import pipeline (separate workstream) | — |
| `src/index.ts` | Composition root: wiring, story mode, object selection, pendingStyle | |
| `src/main.ts` | Demo app chrome: toolbar, stress/docx loaders | |
...
**Tier 1 quick wins** — find & replace (Ctrl+F bar, match counter, prev/next,
...
clipboard both ways). **Tier 1 of the roadmap is complete, and Tier 2 is
complete except .docx export (separate workstream) and print/PDF.**
...
```sh
npm install
npm run dev          # vite, open the printed localhost URL
npm run typecheck
```

- `/?stress=1000` — generate a ~1000-page perf-probe document and log timings.
- `/?docx=<url>` — fetch and import a .docx (import workstream).
...
See **[ROADMAP.md](./ROADMAP.md)** for the detailed Tier 1/Tier 2 plan
(lists, find/replace, hyperlinks, sub/superscript, format painter,
autocorrect, soft line breaks; then .docx export, print/PDF, sections,
columns, header variants, fields/TOC, footnotes). Headline: Tier 1 ≈ 7–8
working days, Tier 2 ≈ 12–14, after which the editor covers ~95% of everyday
Word usage.
...
- No rowSpan (vertical cell merge); no repeat-header-row on table chunks yet.
- Band stories: tables inside headers render but are read-only; a band taller
  than the margin overlaps instead of pushing content.
