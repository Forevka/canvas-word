...
- **Contextual floating toolbars — one framework, priority-based.** The two hand-wired floating
  ...
  30), a new **hyperlink** bar (25 — Open / Edit / Copy / Remove, shown when the caret sits in a
  link), and the **text format** bar (20). Highest priority wins, so a selected image beats a text
  ...
  New editor accessors back it: `linkAtCaret()` (+ position-based `linkAtPosition`). See the
  [`context-toolbars`](./examples/context-toolbars) example.
