...
// live preview) stays EXTERNAL — dev serves it as-is (optimizeDeps.exclude),
// the production build externalizes it and the index.html import map points at
// the copied dist-lib. The `/builder` + `/query` subpaths are aliased to the
// frontend SOURCE in DEV so this example runs against the current tree without a
// lib rebuild (and demonstrates the query/edit API even before it is published);
// the production build externalizes them too, resolved via the import map.
