...
- The **Vite** examples (`embed-live`, `playground`, `query-edit`, `embed-multi`,
  `editor-constructor`) install the package and run under Vite.
- The **static** examples (`embed-offline`, `ribbon-customization`, `custom-fonts`)
...
- `query-edit` needs the **`./query` subpath**, which must be present in the
  published version (`npm view @forevka/wordcanvas exports`) — publish a current
  version if your registry copy predates the query/edit release.
